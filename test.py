        ...
        for url, user, passw, enctype in credentials:
            ...
            if enctype:
                try:
                    ...
                    LOG.warning(
                        "Failed to decode username or password for entry from URL %s", url)
                    LOG.exception(e)
                    continue

            LOG.debug(
                "Decoded username '%s' and password '%s' for website '%s'", user, passw, url)
...
class PassOutputFormat(OutputFormat):
    ...
    def test_pass_cmd(self) -> None:
        ...
        try:
            ...
            if e.errno == 2:
                LOG.error(
                    "Password store is not installed and exporting was requested")
        ...
        if p.returncode != 0:
            if 'Try "pass init"' in p.stderr:
                ...
                LOG.error(
                    "Initialize the password store manually by using 'pass init'")
    ...
    def export(self):
        ...
        for address in self.to_export:
            for user, passw in self.to_export[address].items():
                ...
                cmd: list[str] = [self.cmd, "insert",
                                  "--force", "--multiline", passname]
...
def get_profile(basepath: str, interactive: bool, choice: Optional[str], list_profiles: bool):
    ...
    try:
        ...
        if e.exitcode == Exit.MISSING_PROFILEINI:
            LOG.warning(
                "Continuing and assuming '%s' is a profile location", basepath)
            profile = basepath
        ...
        if not os.path.isdir(profile):
            LOG.error(
                "Profile location '%s' is not a directory. Has profiles.ini been tampered with?", profile)
